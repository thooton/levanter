...
@LmConfig.register_subclass("citrine3")
@dataclass(frozen=True)
class Citrine3Config(LmConfig):
    model_dim: int
    ff_dim: int
    query_count: int
    layer_count: int
...
class Attention(eqx.Module, StateDictSerializationMixin):
    conf: Citrine3Config = eqx.static_field()
    wq: hnn.Linear
    wk: hnn.Linear
    wv: hnn.Linear
    wo: hnn.Linear
...
class Citrine3(eqx.Module, LmHeadModel[Citrine3Config], StateDictSerializationMixin):
    conf: Citrine3Config = eqx.static_field()
    lm_head: hnn.Linear
    ln_p: RMSNorm
    ...
    def resize_vocab(_self, _new_size, _key):
        raise Exception("vocab resize not implemented for Citrine3")
    @staticmethod
    def init(conf, key):
        klm_head, kia, kib, kic, koa, kob, koc = jax.random.split(key, 7)
        ...
        wic = hnn.Linear.init(In=conf.embed_axis, Out=conf.model_axis, key=kic, use_bias=False)
        ...
        woc = hnn.Linear.init(In=conf.model_axis, Out=conf.embed_axis, key=koc, use_bias=False)
        return Citrine3(
            conf, lm_head, ln_p, blocks, ln_f, mask, sin, cos,
            ln_i, wia, wib, wic, ln_o, woa, wob, woc
        )
    @named_call
    ...
    def compute_loss(
        ...
        *,
        ...
        xb = example.tokens[batch_axis, mbsz:2*mbsz]
        xc = example.tokens[batch_axis, 2*mbsz:]
        ma = example.loss_mask[batch_axis, :mbsz]
        mb = example.loss_mask[batch_axis, mbsz:2*mbsz]
        mc = example.loss_mask[batch_axis, 2*mbsz:]
